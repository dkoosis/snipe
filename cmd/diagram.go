...
// renderSVGString runs the resolved d2 binary and captures its SVG output as
// a string, for embedding in a generated doc rather than writing to stdout.
func renderSVGString(bin, d2src string) (string, error) {
	c := exec.Command(bin, "-", "-") // #nosec G204 -- bin resolved via LookPath
	c.Stdin = strings.NewReader(d2src)
...
// emitDoc is emit's sibling for diagram commands migrated to the docs/
// default (sn-l1kh: living documentation over a stdout dump). Explicit
// --format d2/svg behave exactly as emit() today; only the truly-unset
// default changes, writing a self-contained docs/diagrams/<name>.md instead
// of printing D2 source to the terminal.
func emitDoc(name, summary, d2src string) error {
	switch diagramFormat {
	case "d2", "svg":
		return emit(summary, d2src)
	case "":
		return writeDiagramDoc(name, summary, d2src)
	default:
		return fmt.Errorf("unknown --format %q (want d2 or svg)", diagramFormat)
	}
}

// writeDiagramDoc renders docs/diagrams/<name>.md: the Claude-summary text,
// an inline SVG (via the same d2 CLI shell-out renderSVG uses) when d2 is on
// PATH, and the D2 source as a fenced code block underneath. Degrades to
// D2-only with an install pointer — never fails the command — when d2 isn't
// available, since a missing renderer is a degraded-but-valid doc, not a
// broken one.
func writeDiagramDoc(name, summary, d2src string) error {
	root := util.FindProjectRoot(".")
	if root == "" {
		root = "."
	}
	return writeDiagramDocTo(root, name, summary, d2src)
}

// writeDiagramDocTo is writeDiagramDoc with an explicit root, split out so
// tests can target a temp directory instead of walking to the real repo root.
func writeDiagramDocTo(root, name, summary, d2src string) error {
	...
	fmt.Fprintf(&b, "<!-- generated by `snipe diagram %s` — do not hand-edit; rerun to refresh -->\n\n", name)
	b.WriteString("```\n")
	...
	if bin, err := exec.LookPath("d2"); err == nil {
		if svg, rerr := renderSVGString(bin, d2src); rerr == nil {
			b.WriteString(svg)
			b.WriteString("\n\n")
		} else {
			fmt.Fprintf(&b, "_SVG not rendered: %v_\n\n", rerr)
		}
	} else {
		b.WriteString("_SVG not rendered: `d2` CLI not found on PATH — install from https://d2lang.com, then rerun to add the diagram image._\n\n")
	}
...
func runDiagramArch() error {
	...
	return emitDoc("arch", sb.String(), b.Render())
}
